...
$order_id = $_GET['id'] ?? 0;
$stmt = $pdo->prepare("SELECT o.*, u.full_name, u.email, u.phone, u.address FROM bookings o JOIN users u ON o.user_id = u.id WHERE o.id=?");
$stmt->execute([$order_id]);
$order = $stmt->fetch(PDO::FETCH_ASSOC);

$items = [];
if ($order) {
    $stmt = $pdo->prepare("SELECT oi.*, p.name FROM booking_details oi JOIN services p ON oi.product_id = p.id WHERE oi.order_id=?");
    $stmt->execute([$order_id]);
...
<body>
    <div class="container my-5">
        ...
        <?php if ($order): ?>
            ...
            <a href="bookings.php" class="btn btn-secondary">Quay lại</a>
        <?php else: ?>
